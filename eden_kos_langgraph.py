# Eden KOS v0.3 - LangGraph Integrated RAG System with Textual TUI (Offline, Enhanced)
...
def save_memory(memory):
    ...
    trimmed = memory[-100:]  # limit memory size to last 100 entries
    with open(HISTORY_FILE, "w", encoding="utf-8") as f:
        json.dump(trimmed, f, indent=2)

def build_prompt(query, memory, docs):
    past = "\n".join([f"User: {m['user']}\nAssistant: {m['assistant']}" for m in memory[-5:]])
    
    has_context = docs and any(len(doc.page_content.strip()) >= 30 for doc in docs)
    
    if not has_context:
        return f"""You are a helpful assistant. Use the memory of previous conversations to answer. If unsure, say so.
...
"""
        ...
        return f"""You are a helpful assistant. Use the following context and memory to answer. If the answer is directly in the context, cite it. If it's indirectly related, provide your best grounded reasoning.
...
def retrieve(state):
    query = state["input"]
    results = state["vectorstore"].similarity_search(query, k=3)
...
def generate_response(state):
    response = ollama.chat(model=LLM_MODEL, messages=[{"role": "user", "content": state["prompt"]}])
...
class EdenApp(App):
    ...
    def compose(self) -> ComposeResult:
        yield Header()
        yield Container(Input(placeholder="Ask a question...", id="query_input"), Static(id="output_area"))
        yield Footer()
    ...
    async def on_input_submitted(self, event: Input.Submitted) -> None:
        ...
        if query.lower() in {"exit", "quit"}:
            ...
            state = await to_thread(self.graph.invoke, {"input": query, "vectorstore": self.db})
            response = state["output"]
            self.output_widget.update(f"[b]User:[/b] {query}\n[i]Assistant:[/i] {response}\n\n")
