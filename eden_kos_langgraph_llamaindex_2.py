...
class EdenApp(App):
    ...
    def __init__(self, index, graph):
        ...
        self.index = index
        self.graph = graph
        self.output_widget: Log | None = None

    def compose(self) -> ComposeResult:
        yield Header()
        yield Vertical(
            Input(placeholder="Ask a question...", id="query_input"),
            Log(id="output_area", auto_scroll=True),
            id="chat_panel",
        )
        yield Footer()
    ...
    def on_mount(self) -> None:
        ...
        self.output_widget = self.query_one("#output_area", Log)
        self.output_widget.styles.text_wrap = "wrap"
        self.output_widget.styles.text_overflow = "ellipsis"
        # focus the input
    ...
    async def on_input_submitted(self, event: Input.Submitted) -> None:
        ...
        # append and scroll each message
        self.output_widget.write_line(f"User: {query}", scroll_end=True)
        self.output_widget.write_line(f"Assistant: {response}", scroll_end=True)
